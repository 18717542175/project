...
void Font_Test(void)
{
	...
	delay_ms(1000);
	Lcd_Clear(GRAY0);
	Gui_DrawFont_GBK16(16,30,BLUE,GRAY0,"西安科技大学");
	Gui_DrawFont_GBK16(16,50,BLUE,GRAY0,"通信科创班");
	Gui_DrawFont_GBK16(16,70,RED,GRAY0, "东八区小小白");
...
void showimage(const unsigned char *p) //显示40*40 QQ图片
{
	...
	Lcd_SetRegion(128*j,128*k,128*j+127,128*k+127);		//坐标设置
	for(i=0;i<128*128;i++)
 {	
	picL=*(p+i*2);	//数据低位在前
	picH=*(p+i*2+1);				
